...
class UsuarioController {
    ...
    public function atualizaUsuario($id, $nome, $email, $cpf, $telefone, $perfil, $ativo){
        // Processo de validação dos dados recebidos
        if(empty($nome) || !$this->validarNome($nome)){
            throw new InvalidArgumentException("Campo NOME inválido!");
        }
        ...
        return $this->usuarioModel->update($id, $nome, $email, $cpf, $telefone, $perfil, $ativo);
    }

    public function atualizaSenha($id, $password){
        if(empty($id)){
            throw new InvalidArgumentException("Campo ID inválido!");
        }
        if(empty($password) || $this->validarSenha($password)){
            throw new InvalidArgumentException("Campo Pasword inválido!");
        }
        return $this->usuarioModel->updatePassword($id, $password);
    }
